       ...
       t0=clock;
        for i=1:ReportCycle,
           ...
           [f,J]=eval([CurrentSystem,'(data,x,[0;param],v)']);
            ...
            JJ_sp=[   J(sub_strt+1:fn+1,sub_strt:fn)  zeros(2*no_pq,2*no_pq)                                 (J(no_gen+1:n+1,nn:nn))*x_inter
                J(sub_strt+1:fn+1,fn+no_gen:2*fn)  J(sub_strt+1:fn+1,sub_strt:fn)-lambda_sp*eye(2*no_pq)  (J(sub_strt+1:fn+1,fn+nn:fn+nn))*x_inter
            ...
            %x(1:no_gen-1)=(1-alpha_sp)*x_up(1:no_gen-1)+alpha_sp*x_low(1:no_gen-1);
				x(nn:nn)=(1-alpha_sp)*x_up(nn:nn)+alpha_sp*x_low(nn:nn);
